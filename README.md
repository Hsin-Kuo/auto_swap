...
自動執行 SOL ↔ USDC 小額 swap，用於刷鏈上活動量。透過 Jupiter V6 聚合器執行交易，金額與時間間隔皆隨機化。

## 功能

- SOL ↔ USDC 隨機方向來回 swap
- 每筆金額 2~13 USDC 隨機
- 每天約 100 筆，間隔 5~24 分鐘隨機分散
...
開啟瀏覽器前往 `http://localhost:3000`，可查看每日交易紀錄。
...
| `SLIPPAGE_BPS` | 100 | 滑點容忍度（100 = 1%） |
| `MIN_INTERVAL_SEC` | 300 | 最短交易間隔（秒） |
| `MAX_INTERVAL_SEC` | 1440 | 最長交易間隔（秒） |
| `MIN_SOL_RESERVE` | 0.05 | SOL 最低保留量（防 rent/gas 不足） |
...
```
logs/transactions_2026-03-07.csv
```

欄位：

```
timestamp, direction, input_amount, output_amount, input_token, output_token, tx_hash, status, error
```
